...
5. **Tandas JSON**: Las tandas de ejercicios provienen del prompt documentado en `docs/prompts.md`. No editar manualmente el contenido salvo correcciones puntuales de sintaxis JSON. Al generar o incorporar una nueva tanda, verificar y comparar las preguntas únicamente contra las tandas ya cargadas dentro de la/s carpeta/s de la/s materia/s correspondiente/s (`src/assets/data/<materia>/`) para garantizar que no existan preguntas muy similares ni redundancias conceptuales, aun cuando difieran en redacción o formato.
